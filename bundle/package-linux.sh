...
dotnet publish "$project_path" -c Release -r linux-x64 --self-contained true -p:PublishTrimmed=false -p:PublishReadyToRun=false -o "$publish_dir" -p:Version="$version" -p:InformationalVersion="$version"
