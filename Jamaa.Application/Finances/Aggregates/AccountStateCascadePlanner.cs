...
public static class AccountStateCascadePlanner
{
        ...
        bool isActiveTarget)
    {
        ...
        var simulatedState = accountsById.ToDictionary(account => account.Key, account => account.Value.IsActive, StringComparer.Ordinal);
        foreach (var plannedId in plannedIds)
        {
            simulatedState[plannedId] = isActiveTarget;
        }
        ...
        foreach (var ancestorId in GetAncestorIds(accountsById, rootAccountId))
        {
            ...
            var allChildrenMatchTarget = childIds
                .All(childId => simulatedState.TryGetValue(childId, out var childIsActive) && childIsActive == isActiveTarget);
            if (!allChildrenMatchTarget)
            {
                continue;
            }

            if (!simulatedState.TryGetValue(ancestorId, out var ancestorIsActive) || ancestorIsActive == isActiveTarget)
            {
                continue;
            }
