...
public static class FiscalYearUpdatePlanner
{
        ...
        out string? error)
    {
        ...
        if (!TryValidateRequestedRange(requestedStartDate, requestedEndDate, out var startDate, out var endDate,
                out error)) return false;

        if (!TryGetPlanningContext(fiscalYears, targetFiscalYearId, out var context, out error)) return false;
        ...
        if (!TryPlanPreviousBoundary(context, startDate, out var previousPlan, out error)) return false;

        if (!TryPlanNextBoundary(context, endDate, out var nextPlan, out error)) return false;

        if (!TryValidateContiguity(context, startDate, endDate, previousPlan, nextPlan, out error)) return false;
        ...
        out string? error)
    {
        ...
        if (context.Previous is null) return true;
        ...
        if (adjustedPreviousEndDate == context.Previous.EndDate) return true;
        ...
        out string? error)
    {
        ...
        if (context.Next is null) return true;
        ...
        if (adjustedNextStartDate == context.Next.StartDate) return true;
        ...
        FiscalYearWindow? nextPlan)
    {
        return context.OrderedFiscalYears
            .Select(fiscalYear =>
            {
                if (fiscalYear.Id == context.Current.Id) return (StartDate: startDate, EndDate: endDate);

                if (previousPlan is not null && fiscalYear.Id == previousPlan.Id)
                    return (previousPlan.StartDate, previousPlan.EndDate);

                if (nextPlan is not null && fiscalYear.Id == nextPlan.Id) return (nextPlan.StartDate, nextPlan.EndDate);
    ...
    private static bool IsContiguous(IReadOnlyList<(DateTime StartDate, DateTime EndDate)> ranges)
    {
        ...
        if (ranges.Any(range => range.EndDate < range.StartDate)) return false;
