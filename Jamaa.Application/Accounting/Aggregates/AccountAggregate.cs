...
public class AccountAggregate : ReceivePersistentActor
{
    ...
    private void RegisterEventHandlers()
    {
        ...
        {
            if (offer.Snapshot is AccountStateStore state) _state.CopyFrom(state);
        });
    ...
    private void Handle(CreateAccount command)
    {
        ...
        if (!TryNormalizeAccount(command.Code, command.Name, command.Description, out var code, out var name,
                out var description)) return;
        ...
        if (!TryValidateParentAccount(command.ParentId, command.Type, null)) return;
    ...
    private void Handle(UpdateAccount command)
    {
        ...
        if (!TryValidateParentAccount(command.ParentId, command.Type, command.AccountId.Value)) return;
    ...
    private void Handle(DeactivateAccount command)
    {
        ...
        var deactivationEvents = BuildCascadeAccountIds(command.AccountId.Value, false)
    ...
    private void Handle(ReactivateAccount command)
    {
        ...
        var reactivationEvents = BuildCascadeAccountIds(command.AccountId.Value, true)
    ...
    {
        if (!_state.Accounts.TryGetValue(@event.AccountId.Value, out var account)) return;
    ...
    {
        if (_state.Accounts.TryGetValue(@event.AccountId.Value, out var account)) account.IsActive = false;
    }

    private void Apply(AccountReactivated @event)
    {
        if (_state.Accounts.TryGetValue(@event.AccountId.Value, out var account)) account.IsActive = true;
    }
    ...
    {
        if (parentAccountId is null) return true;
        ...
        if (currentAccountId is null) return true;
    ...
    {
        if (_hasHydratedFromPersistence) return;
        ...
        {
            if (_state.Accounts.ContainsKey(persistedAccount.Id.Value)) continue;

            _state.Accounts[persistedAccount.Id.Value] = new AccountState
            {
                Id = persistedAccount.Id.Value,
    ...
    private sealed class AccountStateStore
    {
        ...
        public AccountStateStore Clone()
        {
            ...
            foreach (var account in Accounts) clone.Accounts[account.Key] = account.Value.Clone();
        ...
        {
            Accounts.Clear();
            foreach (var account in other.Accounts) Accounts[account.Key] = account.Value.Clone();
        }
    }
