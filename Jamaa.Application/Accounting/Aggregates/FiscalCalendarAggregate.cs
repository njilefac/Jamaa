...
public class FiscalCalendarAggregate : ReceivePersistentActor
{
    ...
    private void RegisterEventHandlers()
    {
        ...
        {
            if (offer.Snapshot is FiscalCalendarState state) _state.CopyFrom(state);
        });
    ...
    {
        if (!TryGetValidFiscalYearRange(command.StartDate, command.EndDate, out var startDate, out var endDate)) return;

        if (!TryValidateFiscalYearCreation(command.FiscalYearId.Value, startDate, endDate)) return;
    ...
    {
        if (!TryGetExistingFiscalYear(command.FiscalYearId.Value, out var existingFiscalYear)) return;

        if (!TryGetValidFiscalYearRange(command.StartDate, command.EndDate, out var startDate, out var endDate)) return;

        if (!TryGetFiscalYearUpdatePlan(command, startDate, endDate, out var plan)) return;
        ...
        bool isLocked)
    {
        ...
        foreach (var periodId in deletedPeriodIds) fiscalYear.Periods.Remove(periodId);
    }
    ...
    private void Handle(DeleteFiscalYear command)
    {
        ...
        if (!TryValidateFiscalYearDeletion(command.FiscalYearId.Value)) return;
    ...
    {
        if (_state.FiscalYears.TryGetValue(fiscalYearId, out fiscalYear!)) return true;
    ...
    private bool TryGetValidFiscalYearRange(DateTime requestedStartDate, DateTime requestedEndDate,
    ...
    private bool TryValidateFiscalYearCreation(string fiscalYearId, DateTime startDate, DateTime endDate)
    {
        ...
        if (IsContiguous(candidateFiscalYears)) return true;
    ...
    private bool TryGetFiscalYearUpdatePlan(UpdateFiscalYear command, DateTime startDate, DateTime endDate,
        out FiscalYearUpdatePlanner.FiscalYearUpdatePlan plan)
    {
    ...
    private void PersistOptionalFiscalYearUpdate(OrganisationId organisationId,
        FiscalYearUpdatePlanner.FiscalYearWindow? plannedFiscalYear)
    {
        if (plannedFiscalYear is null ||
            !_state.FiscalYears.TryGetValue(plannedFiscalYear.Id, out var fiscalYear)) return;
    ...
    private bool TryValidateFiscalYearDeletion(string fiscalYearId)
    {
        ...
        if (remaining.Count <= 1 || IsContiguous(remaining)) return true;
    ...
    {
        Persist(new AccountingPeriodDeleted(command.OrganisationId, command.FiscalYearId, command.AccountingPeriodId),
            Apply);
        PersistFiscalYearLockSync(command.OrganisationId, fiscalYear.Id);
    }
    ...
    {
        if (!_state.FiscalYears.TryGetValue(fiscalYearId, out var fiscalYear)) return;

        if (fiscalYear.Periods.Count == 0) return;
    ...
    {
        if (!_state.FiscalYears.TryGetValue(@event.FiscalYearId.Value, out var fiscalYear)) return;
    ...
    private void Apply(AccountingPeriodUpdated @event)
    {
        ...
        if (!fiscalYear.Periods.TryGetValue(@event.AccountingPeriodId.Value, out var period)) return;
    ...
    private void Apply(FiscalYearPeriodsRegenerated @event)
    {
        ...
        foreach (var deletedPeriodId in @event.DeletedPeriodIds) fiscalYear.Periods.Remove(deletedPeriodId);
    ...
    {
        if (!IsContiguous(periods)) return false;
    ...
    private static bool IsContiguous(List<DateRange> ranges)
    {
        ...
        for (var index = 1; index < ordered.Count; index++)
        {
            ...
            if (current.StartDate != previous.EndDate.AddDays(1)) return false;
        }
    ...
    private sealed class FiscalCalendarState
    {
        ...
        public FiscalCalendarState Clone()
        {
            ...
            foreach (var fiscalYear in FiscalYears) clone.FiscalYears[fiscalYear.Key] = fiscalYear.Value.Clone();
        ...
        {
            FiscalYears.Clear();
            foreach (var fiscalYear in state.FiscalYears) FiscalYears[fiscalYear.Key] = fiscalYear.Value.Clone();
        }
    }
    ...
    private sealed class FiscalYearState
    {
        ...
        public FiscalYearState Clone()
        {
            ...
            foreach (var period in Periods) clone.Periods[period.Key] = period.Value.Clone();
