...
{
    public async Task<User> CreateUser(string username, string password, string email, string? firstName,
        string? lastName)
    {
        logger.LogInformation("creating user account...");
        var newUser = new User(username, password, email, firstName, null, lastName, false);
